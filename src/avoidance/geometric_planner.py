...
class GeometricPlanner(BaseAvoidancePlanner):
    ...
    Calculates drone movements (yaw and altitude) to maintain a safe distance from obstacles.
    """
    def __init__(self, safe_distance: float = 150.0, time_horizon: float = 2.0, frame_width: int = 640, frame_height: int = 640):
        """
        Initialize the planner.
        
        Args:
            safe_distance: Minimum distance (in pixels) to maintain from obstacle centers.
            time_horizon: Seconds to look ahead for potential collisions.
    ...
    def plan(self, tracks: List[Track]) -> Tuple[float, float]:
        """
        Calculates (yaw_delta, altitude_delta) based on current tracks and their velocities.
        
        Args:
            tracks: List of currently tracked objects.
            
        Returns:
            Tuple of (yaw_delta, altitude_delta) in degrees or normalized units.
        """
        if not tracks:
            return 0.0, 0.0
        ...
        for track in tracks:
            ...
            dist = np.sqrt((future_cx - frame_center_x)**2 + (future_cy - frame_center_y)**2)
